// =============================================================================
// DealDetail — the core deal-management screen. Tabs cover overview,
// milestones, rehab line items, contractors, expenses, documents, and notes.
//
// This file owns the rehab assignment flows, the close-deal wizard (sold +
// convert-to-rental paths), milestone tracking, and inline contractor / bid
// editing. RehabItemDetail handles the per-line drill-down and is rendered
// outside this component, so it isn't imported here.
// =============================================================================
import { useState, useEffect, useMemo } from "react";
import {
  Plus, X, Pencil, Trash2, ChevronLeft, ChevronRight, Check, CheckCircle, Circle, Wrench, Truck,
...
import {
  createDealNote as dbCreateDealNote,
...
export function DealDetail({ deal, onBack, backLabel, allDeals, setAllFlips, onNavigateToExpense, onNavigateToContractor, onNavigateToRehabItem, initialTab, onConvertToRental, onDealUpdated, onNavigateToDeal }) {
  ...
  // Union of canonical categories + any custom strings already in use across rehabs
    ...
    <div>
          ...
          <div style={{ textAlign: "right" }}>
              ...
              {stage !== "Converted to Rental" && stage !== "Sold" && (() => {
                ...
                return (<>
                  ...
                  <button onClick={() => {
                    ...
                    <CheckCircle size={12} /> Close Rehab
                  </button>
                </>);
              })()}
            </div>
            ...
            {stage !== "Sold" && stage !== "Converted to Rental" && deal.arvUpdatedAt && (() => {
              ...
              return <p style={{ color: isStale ? "#c2410c" : "var(--text-muted)", fontSize: 12, marginTop: 2 }}>
                {isStale ? "⚠ ARV may be outdated — edit rehab to update" : `ARV as of ${daysAgo(deal.arvUpdatedAt)}`}
              </p>;
            })()}
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <div style={{ background: "var(--surface)", borderRadius: 16, padding: 24, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid var(--border-subtle)" }}>
          <h3 style={{ color: "var(--text-primary)", fontSize: 16, fontWeight: 700, marginBottom: 16 }}>Rehab Profit &amp; Loss</h3>
          {[
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
            <span style={{ fontSize: 13, color: "var(--text-muted)", display: "flex", alignItems: "center" }}>ROI on cash invested<InfoTip text="Net Profit ÷ Total Cash Invested × 100. Measures return as a percentage of the actual cash you put into the rehab." /></span>
      ...
      <div style={{ background: "var(--surface)", borderRadius: 16, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid var(--border-subtle)" }}>
        ...
        <div style={{ display: "flex", gap: 20, marginTop: 12 }}>
          <div style={{ flex: 1, background: "var(--surface-alt)", borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
            <p style={{ color: "var(--text-muted)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 2, display: "flex", alignItems: "center", justifyContent: "center" }}>Budget<InfoTip text="Sum of budgeted amounts across all rehab line items for this rehab." /></p>
          ...
          <div style={{ flex: 1, background: "var(--surface-alt)", borderRadius: 10, padding: "10px 14px", textAlign: "center" }}>
            <p style={{ color: "var(--text-muted)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 2, display: "flex", alignItems: "center", justifyContent: "center" }}>Items<InfoTip text="Rehab line items marked complete out of the total count for this rehab." /></p>
      ...
      {activeTab === "plan" && (
        <div>
          ...
          {/* Empty state with templates if both columns are empty */}
          {rehabItems.length === 0 && milestones.filter(m => m.id).length === 0 ? (
            <div style={{ background: "var(--surface)", borderRadius: 16, padding: 32, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid var(--border-subtle)", marginBottom: 16 }}>
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 13, color: "var(--text-secondary)", maxWidth: 460, margin: "0 auto" }}>Pick a template to pre-populate standard scopes with suggested budgets, or add items one at a time from the column headers below.</p>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
              ...
              <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border-subtle)", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 14px", borderBottom: "1px solid var(--border-subtle)" }}>
                  <Flag size={14} color="var(--c-blue)" />
                  <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", flex: 1, margin: 0 }}>Milestones</h3>
                  <span style={{ background: "var(--surface-muted)", color: "var(--text-label)", borderRadius: 12, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>{doneCount}/{milestones.length} done</span>
                  <button onClick={() => { setEditingMilestoneId(null); setMilestoneForm(emptyMilestone); setShowMilestoneModal(true); }} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "5px 10px", fontWeight: 600, fontSize: 12, cursor: "pointer", color: "var(--text-label)", display: "flex", alignItems: "center", gap: 4 }}>
                    <Plus size={12} /> Add
                  </button>
                </div>
                {(() => {
                  const visibleMs = milestones.map((m, idx) => ({ m, idx })).filter(({ m }) => showCompletedMilestones || !m.done);
                  if (visibleMs.length === 0) {
                    return (
                      <div style={{ padding: 24, textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>
                        {milestones.length === 0 ? "No milestones yet — add one to start tracking" : "All milestones complete"}
                      </div>
                    );
                  }
                  return visibleMs.map(({ m, idx }, vi) => {
                    const overdue = !m.done && m.targetDate && m.targetDate < today;
                    const fmtDate = (d) => new Date(d).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
                    const dateLine = m.done && m.date
                      ? `Completed ${fmtDate(m.date)}`
                      : overdue
                      ? `Overdue — target ${fmtDate(m.targetDate)}`
                      : m.targetDate
                      ? `Target ${fmtDate(m.targetDate)}`
                      : "No target date";
                    return (
                      <div key={m.id || `ms-${idx}`} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderTop: vi > 0 ? "1px solid var(--border-subtle)" : "none" }}
                        onMouseEnter={e => { const a = e.currentTarget.querySelector(".row-actions"); if (a) a.style.opacity = 1; }}
                        onMouseLeave={e => { const a = e.currentTarget.querySelector(".row-actions"); if (a) a.style.opacity = 0; }}>
                        <button onClick={async () => {
                          ...
                          if (!m.done) {
                            ...
                            try {
                              ...
                              setMilestones(prev => prev.map((mm, i) => i === idx ? { ...mm, done: true, date } : mm));
                              const gi = DEAL_MILESTONES.findIndex(mm => mm.id === target?.id);
                            ...
                            try {
                              ...
                              setMilestones(prev => prev.map((mm, i) => i === idx ? { ...mm, done: false, date: null } : mm));
                              const gi = DEAL_MILESTONES.findIndex(mm => mm.id === target?.id);
                          ...
                          style={{ width: 20, height: 20, borderRadius: "50%", border: m.done ? "none" : "1.5px solid var(--border-strong)", background: m.done ? "#1a7a4a" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, padding: 0 }}>
                          {m.done && <Check size={12} color="#fff" strokeWidth={3} />}
                        </button>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 600, color: m.done ? "var(--text-muted)" : "var(--text-primary)", textDecoration: m.done ? "line-through" : "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: 0 }}>{m.label}</p>
                          <p style={{ fontSize: 11, color: overdue ? "var(--c-red)" : "var(--text-muted)", marginTop: 2, fontWeight: overdue ? 600 : 400, margin: "2px 0 0" }}>{dateLine}</p>
                        </div>
                        <div className="row-actions" style={{ display: "flex", gap: 3, flexShrink: 0, opacity: 0, transition: "opacity 0.15s" }}>
                          <button onClick={(e) => { e.stopPropagation(); openEditMilestone(m, idx); }} style={{ background: "var(--surface-muted)", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: "var(--text-label)", display: "flex", alignItems: "center" }} title="Edit"><Pencil size={12} /></button>
                          <button onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ type: "milestone", item: m, index: idx }); }} style={{ background: "var(--danger-badge)", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: "var(--c-red)", display: "flex", alignItems: "center" }} title="Delete"><Trash2 size={12} /></button>
                        </div>
                      </div>
                    );
                  });
                ...
                {doneCount > 0 && (
                  <div style={{ borderTop: "1px solid var(--border-subtle)", padding: "8px 14px" }}>
                    <button onClick={() => setShowCompletedMilestones(v => !v)} style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: 12, cursor: "pointer", padding: 0, fontWeight: 600 }}>
                      {showCompletedMilestones ? "Hide" : "Show"} completed ({doneCount})
                    </button>
                  </div>
                )}
              </div>

              {/* Right column — Scope items */}
              <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border-subtle)", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 14px", borderBottom: "1px solid var(--border-subtle)" }}>
                  <Wrench size={14} color="#9a3412" />
                  <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", flex: 1, margin: 0 }}>Scope items</h3>
                  <span style={{ background: "var(--surface-muted)", color: "var(--text-label)", borderRadius: 12, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>{rehabComplete}/{rehabItems.length} done</span>
                  <button onClick={() => { setEditingRehabIdx(null); setRehabForm(emptyRehab); setShowAddRehab(true); }} style={{ background: "#e95e00", color: "#fff", border: "none", borderRadius: 8, padding: "5px 10px", fontWeight: 600, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                    <Plus size={12} /> Add
                  </button>
                </div>
                {rehabItems.length === 0 ? (
                  <div style={{ padding: 24, textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>No scope items yet — add one to start tracking budget</div>
                ) : rehabItems.map((r, idx) => {
                  const budget = r.budgeted || 0;
                  const spent = r.spent || 0;
                  const pct = budget > 0 ? Math.min(100, (spent / budget) * 100) : 0;
                  const overBudget = spent > budget && r.status !== "complete";
                  const overage = spent - budget;
                  const statusLabel = overBudget ? "Over Budget" : (r.status || "pending");
                  const statusFg = overBudget ? "var(--c-red)" : statusColors[r.status] || statusColors.pending;
                  const statusBgC = overBudget ? "var(--danger-badge)" : statusBg[r.status] || statusBg.pending;
                  const barColor = overBudget ? "var(--c-red)" : r.status === "complete" ? "#1a7a4a" : "#e95e00";
                  return (
                    <div key={r.id || `re-${idx}`} style={{ display: "flex", flexDirection: "column", gap: 6, padding: "12px 14px", borderTop: idx > 0 ? "1px solid var(--border-subtle)" : "none", borderLeft: overBudget ? "3px solid var(--c-red)" : "3px solid transparent", cursor: onNavigateToRehabItem ? "pointer" : "default", transition: "background 0.15s" }}
                      onClick={() => { if (onNavigateToRehabItem) onNavigateToRehabItem(idx); }}
                      onMouseEnter={e => { const a = e.currentTarget.querySelector(".row-actions"); if (a) a.style.opacity = 1; e.currentTarget.style.background = "var(--surface-alt)"; }}
                      onMouseLeave={e => { const a = e.currentTarget.querySelector(".row-actions"); if (a) a.style.opacity = 0; e.currentTarget.style.background = "transparent"; }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <p style={{ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 600, color: overBudget ? "var(--c-red)" : "var(--text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: 0 }}>{r.category}</p>
                        <button onClick={(e) => { e.stopPropagation(); cycleRehabStatus(idx); }} title="Click to cycle status" style={{ background: statusBgC, color: statusFg, borderRadius: 12, padding: "2px 8px", fontSize: 10, fontWeight: 700, border: "none", cursor: "pointer", flexShrink: 0, textTransform: "uppercase", letterSpacing: "0.04em", whiteSpace: "nowrap" }}>{statusLabel}</button>
                        <div className="row-actions" style={{ display: "flex", gap: 3, flexShrink: 0, opacity: 0, transition: "opacity 0.15s" }}>
                          <button onClick={(e) => { e.stopPropagation(); openEditRehab(r, idx); }} style={{ background: "var(--surface-muted)", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: "var(--text-label)", display: "flex", alignItems: "center" }} title="Edit"><Pencil size={12} /></button>
                          <button onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ type: "rehab", item: r, index: idx }); }} style={{ background: "var(--danger-badge)", border: "none", borderRadius: 6, padding: "4px 6px", cursor: "pointer", color: "var(--c-red)", display: "flex", alignItems: "center" }} title="Delete"><Trash2 size={12} /></button>
                        </div>
                      </div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11 }}>
                        <span style={{ color: overBudget ? "var(--c-red)" : "var(--text-secondary)", fontWeight: 600 }}>{fmt(spent)} <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>/ {fmt(budget)}</span></span>
                        {overBudget && <span style={{ color: "var(--c-red)", fontWeight: 600 }}>+{fmt(overage)} over</span>}
                      </div>
                    </div>
                  );
                })}
        ...
        <div>
            ...
            {Object.keys(FLIP_EXPENSE_GROUPS).map(group => {
              ...
              return (
                <div key={group} style={{ background: "var(--surface)", borderRadius: 12, padding: "12px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid var(--border-subtle)", minWidth: 130, flex: "1 0 auto" }}>
                  <p style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, textTransform: "uppercase", marginBottom: 4, whiteSpace: "nowrap", display: "flex", alignItems: "center" }}>{group}<InfoTip text={`Sum of ${group} expenses for this rehab. Includes categories: ${subs.join(", ")}.`} /></p>
              ...
              <div style={{ textAlign: "center", padding: 48, color: "var(--text-muted)" }}>
                ...
                {hasExpFilters ? (
                  ...
                  <>
                    ...
                    <p style={{ fontSize: 13 }}>Click &ldquo;Add Expense&rdquo; to start tracking spend for this rehab.</p>
                  </>
                )}
              </div>
            ) : (
          ...
          {onNavigateToExpense && flipExpenses.length > 0 && (
            <button onClick={() => onNavigateToExpense(flipExpenses[0].id)} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#e95e00", fontSize: 13, fontWeight: 600, cursor: "pointer", padding: "12px 0 0", marginLeft: "auto" }}>
              View all expenses across rehabs <ChevronRight size={14} />
            </button>
          )}
          {showExpenseModal && (() => {
            const PaidToDropdown = () => {
              const q = (expForm.vendor || "").toLowerCase();
              // Contractors on this rehab — show all on empty, filter on query
      ...
      {activeTab === "contractors" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>{flipContractors.length} contractor{flipContractors.length !== 1 ? "s" : ""} on this rehab</p>
                  ...
                  <div key={c.id}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                      <div style={{ background: "var(--surface-alt)", borderRadius: 10, padding: "10px 12px" }}>
                        <p style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, textTransform: "uppercase", marginBottom: 3, display: "flex", alignItems: "center" }}>Bids (This Rehab)<InfoTip text="Number of bids this contractor has submitted on this rehab. Pending count shown in parentheses if any are awaiting acceptance." /></p>
                        <p style={{ color: "var(--text-primary)", fontSize: 13, fontWeight: 700 }}>{t.bidCount}{t.pendingBids > 0 ? ` (${t.pendingBids} pending)` : ""}</p>
                      </div>
                      <div style={{ background: "var(--surface-alt)", borderRadius: 10, padding: "10px 12px" }}>
                        <p style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, textTransform: "uppercase", marginBottom: 3, display: "flex", alignItems: "center" }}>Total Bid<InfoTip text="Sum of all accepted bid amounts from this contractor on this rehab." /></p>
                      ...
                      <div style={{ background: t.owed > 0 ? "var(--warning-bg)" : t.totalBid > 0 ? "var(--success-badge)" : "var(--surface-alt)", borderRadius: 10, padding: "10px 12px" }}>
                        <p style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 600, textTransform: "uppercase", marginBottom: 3, display: "flex", alignItems: "center" }}>Balance Owed<InfoTip text="Total Bid − Paid to Date. What you still owe this contractor on this rehab." /></p>
        ...
        return (
          <Modal title={`Record Payment — ${con.name}`} onClose={closePayment}>
            <div style={{ background: "var(--surface-alt)", borderRadius: 12, padding: 14, marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ fontSize: 13, color: "var(--text-secondary)" }}>Total Bid (This Rehab)</span>
        ...
        <div>
          ...
          {showNoteInput && (
            <div style={{ background: "var(--surface)", borderRadius: 16, padding: 20, marginBottom: 16, border: "1px solid var(--border-subtle)", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
              <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Add a note about this rehab... e.g. 'Spoke with inspector, needs structural review on back wall'" rows={3} style={{ ...iS, resize: "vertical", fontFamily: "inherit" }} autoFocus />
          ...
          <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border-subtle)", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", overflow: "hidden" }}>
            {dealNotes.length === 0 ? (
              <div style={{ textAlign: "center", padding: 48, color: "var(--text-muted)" }}>
                ...
                <p style={{ fontSize: 13 }}>Keep a running journal of updates, calls, and decisions for this rehab.</p>
              </div>
            ) : filtered.length === 0 ? (
      ...
      {showEditDeal && (
        <Modal title="Edit Rehab" onClose={() => setShowEditDeal(false)}>
          <div style={{ maxHeight: "60vh", overflowY: "auto", paddingRight: 4 }}>
            {[
              { label: "Rehab Name", key: "name", type: "text", placeholder: "e.g. Oakdale Craftsman" },
      ...
      {showReopenConfirm && (
        <Modal title="Reopen Rehab" onClose={() => setShowReopenConfirm(false)} width={460}>
          <p style={{ color: "var(--text-label)", fontSize: 14, marginBottom: 12 }}>
            Reopen this rehab? Stage will reset to <strong>Listed</strong> so you can update from there.
          </p>
          {stage === "Sold" ? (
            <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 18 }}>
              Sale data (price, close date, net profit) is preserved on the rehab. Update or replace it once you re-sell.
            </p>
          ) : (
            <p style={{ color: "#9a3412", fontSize: 13, marginBottom: 18, padding: "10px 12px", background: "var(--warning-bg)", borderRadius: 8 }}>
              The rental property created when you converted this rehab still exists in your portfolio. Reopening only affects this rehab — manage the rental separately if you need to.
            </p>
          )}
          <div style={{ display: "flex", gap: 10 }}>
      ...
      {showCloseDeal && (
        <Modal title={closeDealStep === "choose" ? "Close Rehab" : closeDealStep === "sold" ? "Mark as Sold" : "Convert to Rental"} onClose={() => setShowCloseDeal(false)}>
          {/* Step 1: Choose path */}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 8 }}>
              <button onClick={() => setCloseDealStep("sold")} style={{ display: "flex", alignItems: "center", gap: 14, padding: 18, background: "var(--success-tint)", border: "2px solid #9fcfb4", borderRadius: 14, cursor: "pointer", textAlign: "left", transition: "border-color 0.15s" }} onMouseEnter={e => e.currentTarget.style.borderColor = "#1a7a4a"} onMouseLeave={e => e.currentTarget.style.borderColor = "var(--success-border)"}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-secondary)" }}>Enter sale price, close date, and selling costs to finalize the rehab</p>
                </div>
              </button>
              {onConvertToRental && (
          ...
          {closeDealStep === "sold" && (<>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => {
                ...
                pushDealNote(`Rehab closed — sold for ${fmt(sp)} with net profit of ${fmt(netProfit)}.`);
                if (onDealUpdated) onDealUpdated();
          ...
          {closeDealStep === "convert" && (<>
            ...
            <p style={{ color: "var(--text-label)", fontSize: 14, marginBottom: 16 }}>
              Convert this rehab into a rental property in your portfolio. The deal will be marked as "Converted to Rental" and a new property will be created with the details below.
            </p>
            ...
            <div style={{ background: "var(--warning-bg)", borderRadius: 10, padding: 12, marginBottom: 20, border: "1px solid #fdba74" }}>
              ...
              <p style={{ fontSize: 12, color: "#9a3412", marginTop: 4 }}>
                You'll be taken to the Add Property form pre-filled with this rehab's info. You can review and adjust the details (rent amount, loan info, etc.) before saving.
              </p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowCloseDeal(false)} style={{ flex: 1, padding: "12px", border: "1px solid var(--border)", borderRadius: 10, background: "var(--surface)", color: "var(--text-label)", fontWeight: 600, cursor: "pointer" }}>Cancel</button>
