    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "var(--text-secondary)", fontSize: 15 }}>Track every rehab from contract to close</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        <StatCard icon={Hammer}     label="Active Rehabs"     value={activeDeals.length}              sub="In pipeline"      color="#e95e00"        tip="Number of deals not yet sold." />
        <StatCard icon={DollarSign} label="Capital Deployed" value={fmtK(totalDeployed)}             sub="Purchase + rehab" color="var(--c-blue)"  tip="Sum of purchase price + rehab spent across active rehabs." />
        <StatCard icon={TrendingUp} label="Projected Profit" value={fmtK(Math.round(totalProjected))} sub="Active rehabs"     color="var(--c-green)" tip="ARV − Purchase − Rehab Budget − Estimated Holding & Selling Costs across active rehabs." />
        <StatCard icon={Star}       label="Realized Profit"  value={fmt(realizedProfit)}             sub="Closed rehabs YTD" color="var(--c-purple)" tip="Actual net profit from deals closed/sold this year." />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 20 }}>
        {filtered.map(f => <DealCard key={f.id} rehab={f} onSelect={onSelect} />)}
        {filtered.length === 0 && (
          <div style={{ gridColumn: "1 / -1" }}>
            {DEALS.length === 0
              ? <EmptyState icon={Layers} title="No rehabs yet" subtitle="Add your first rehab to start tracking your pipeline." actionLabel="Add Rehab" onAction={onGuidedSetup} />
              : <EmptyState icon={Search} title="No rehabs match this filter" subtitle="Try selecting a different stage or clear the filter." />
            }
          </div>
        )}
      </div>
