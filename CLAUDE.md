...
- [x] **Profile-backed plan + onboarding flag** — `auth.jsx` now fetches `public.profiles` on auth change and exposes `user.plan`, `user.hasOnboarded`, `user.subscriptionStatus`, `user.currentPeriodEnd`, `user.stripeCustomerId`. Pre-trigger users get a profile row auto-inserted by an `on_auth_user_created` trigger; legacy users get one upserted by AuthProvider as a fallback. The hardcoded `plan: "pro"` is gone.
- [x] **Onboarding gate fix** — `AppShell.jsx` now gates on `user.hasOnboarded === false`. On wizard completion, `profiles.has_onboarded` flips to `true` (persists across sessions). Demo account is excluded.
...
- [ ] **Stripe billing wiring** — DB ready (`stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `current_period_end` columns + indexes). Still needed: Stripe account + product/price, Supabase Edge Function for the webhook, `create-checkout-session` + Customer Portal endpoints, wire the two Settings buttons. Until then, every user is `subscription_status = null` and "Subscribe" is a no-op.
- [ ] **Email confirmation + custom SMTP** — Supabase dashboard config: enable "Confirm email", set Site URL + Redirect URLs to the Vercel domain, wire custom SMTP via Resend or SendGrid. No code changes needed here unless we want a "check your email" interstitial after signup (currently just a success Alert).
