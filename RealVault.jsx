...
function PropertyDetail({ property, onBack, onEditProperty, onGoToTransactions, onNavigateToTransaction, onNavigateToTenant }) {
  ...
  const propExpenses = propTransactions.filter(t => t.type === "expense");

  // Expense tab state
  const [showExpModal, setShowExpModal] = useState(false);
  const [expEditId, setExpEditId] = useState(null);
  const [expDeleteConfirm, setExpDeleteConfirm] = useState(null);
  ...
  const emptyExp = { date: new Date().toISOString().slice(0, 10), category: "General Maintenance", description: "", amount: "", payee: "" };
  const [expForm, setExpForm] = useState(emptyExp);
  const sef = k => e => setExpForm(f => ({ ...f, [k]: e.target.value }));
  const [payeeFocus, setPayeeFocus] = useState(false);

  const allPayees = useMemo(() => {
    const names = new Set(TRANSACTIONS.filter(t => t.type === "expense" && t.payee).map(t => t.payee));
    return [...names].filter(Boolean).sort();
  }, []);

  const filteredExp = useMemo(() => {
    let list = propExpenses;
    if (expCatFilter !== "all") list = list.filter(t => t.category === expCatFilter);
    if (expSearch) { const q = expSearch.toLowerCase(); list = list.filter(t => (t.description || "").toLowerCase().includes(q) || (t.category || "").toLowerCase().includes(q) || (t.payee || "").toLowerCase().includes(q)); }
    if (expDateFilter !== "all") {
      const now = new Date(); const y = now.getFullYear(); const m = now.getMonth();
      let from, to;
      if (expDateFilter === "thisMonth")  { from = new Date(y, m, 1); to = new Date(y, m + 1, 0); }
      if (expDateFilter === "lastMonth")  { from = new Date(y, m - 1, 1); to = new Date(y, m, 0); }
      if (expDateFilter === "thisYear")   { from = new Date(y, 0, 1); to = new Date(y, 11, 31); }
      if (expDateFilter === "lastYear")   { from = new Date(y - 1, 0, 1); to = new Date(y - 1, 11, 31); }
      if (expDateFilter === "custom")     { from = expDateFrom ? new Date(expDateFrom) : null; to = expDateTo ? new Date(expDateTo) : null; }
      if (from || to) list = list.filter(t => { const d = new Date(t.date); return (!from || d >= from) && (!to || d <= to); });
    }
    return list.sort((a, b) => b.date.localeCompare(a.date));
  }, [propExpenses, expSearch, expCatFilter, expDateFilter, expDateFrom, expDateTo]);

  const expHasFilters = expSearch || expCatFilter !== "all" || expDateFilter !== "all";
  const clearExpFilters = () => { setExpSearch(""); setExpCatFilter("all"); setExpDateFilter("all"); setExpDateFrom(""); setExpDateTo(""); };
  const expTotal = filteredExp.reduce((s, t) => s + Math.abs(t.amount), 0);
  const expCategories = [...new Set(propExpenses.map(t => t.category))].sort();

  // Expense monthly average
  const expMonthlyAvg = (() => {
    if (propExpenses.length === 0) return 0;
    const dates = propExpenses.map(t => new Date(t.date));
    const earliest = new Date(Math.min(...dates));
    const months = Math.max(1, Math.ceil((new Date() - earliest) / (30.44 * 86400000)));
    return Math.round(propExpenses.reduce((s, t) => s + Math.abs(t.amount), 0) / months);
  })();

  // Largest expense category
  const expByCategory = {};
  propExpenses.forEach(t => { expByCategory[t.category] = (expByCategory[t.category] || 0) + Math.abs(t.amount); });
  const topExpCat = Object.entries(expByCategory).sort((a, b) => b[1] - a[1])[0];

  const handleExpSave = () => {
    if (!expForm.description.trim() || !expForm.amount) return;
    if (expEditId !== null) {
      const idx = TRANSACTIONS.findIndex(t => t.id === expEditId);
      if (idx !== -1) {
        TRANSACTIONS[idx] = { ...TRANSACTIONS[idx], date: expForm.date, category: expForm.category, description: expForm.description.trim(), amount: Math.abs(parseFloat(expForm.amount)), payee: expForm.payee.trim() };
      }
    } else {
      TRANSACTIONS.unshift({
        id: newId(), date: expForm.date, property: property.name, type: "expense",
        category: expForm.category, description: expForm.description.trim(),
        amount: Math.abs(parseFloat(expForm.amount)), payee: expForm.payee.trim(),
      });
    }
    setExpForm(emptyExp); setExpEditId(null); setShowExpModal(false);
    rerender();
  };

  const openExpEdit = (t) => {
    setExpEditId(t.id);
    setExpForm({ date: t.date, category: t.category, description: t.description, amount: String(Math.abs(t.amount)), payee: t.payee || "" });
    setShowExpModal(true);
  };

  const handleExpDelete = () => {
    if (!expDeleteConfirm) return;
    const idx = TRANSACTIONS.findIndex(t => t.id === expDeleteConfirm.id);
  ...
  const tabs = [
    ...
    { id: "expenses", label: "Expenses", icon: DollarSign, count: propExpenses.length },
    ...
    <div>
      ...
      {activeTab === "expenses" && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
            {[
              { label: "Total Expenses", value: fmt(propExpenses.reduce((s, t) => s + Math.abs(t.amount), 0)), color: "#ef4444", icon: DollarSign, tip: "Sum of all expense transactions for this property." },
              { label: "Monthly Average", value: fmt(expMonthlyAvg), color: "#3b82f6", icon: Calendar, tip: "Total expenses divided by the number of months since the first expense was logged." },
              { label: "Expense Count", value: propExpenses.length, color: "#8b5cf6", icon: Receipt, tip: "Total number of expense transactions logged for this property." },
              { label: "Top Category", value: topExpCat ? topExpCat[0] : "—", color: "#f59e0b", icon: Tag, sub: topExpCat ? fmt(topExpCat[1]) : "", tip: "Expense category with the highest total spend for this property." },
            ].map((m, i) => (
              <div key={i} style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 4 }}>
          ...
          {/* Filter bar */}
          <div style={{ display: "flex", gap: 10, marginBottom: expHasFilters ? 10 : 20, flexWrap: "wrap", alignItems: "center" }}>
            <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
              <Search size={14} color="#94a3b8" style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)" }} />
              <input value={expSearch} onChange={e => setExpSearch(e.target.value)} placeholder="Search expenses..." style={{ width: "100%", border: "1.5px solid #e2e8f0", borderRadius: 10, padding: "9px 12px 9px 32px", fontSize: 13, color: "#0f172a", background: "#fff", outline: "none", boxSizing: "border-box" }} />
            </div>
            <select value={expCatFilter} onChange={e => setExpCatFilter(e.target.value)} style={{ border: "1.5px solid #e2e8f0", borderRadius: 10, padding: "9px 12px", fontSize: 13, color: "#475569", background: "#fff" }}>
              <option value="all">All Categories</option>
              {expCategories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select value={expDateFilter} onChange={e => setExpDateFilter(e.target.value)} style={{ border: "1.5px solid #e2e8f0", borderRadius: 10, padding: "9px 12px", fontSize: 13, color: "#475569", background: "#fff" }}>
            ...
            {expDateFilter === "custom" && (
              <>
                <input type="date" value={expDateFrom} onChange={e => setExpDateFrom(e.target.value)} style={{ border: "1.5px solid #e2e8f0", borderRadius: 10, padding: "9px 12px", fontSize: 13, color: "#475569", background: "#fff" }} />
                <input type="date" value={expDateTo} onChange={e => setExpDateTo(e.target.value)} style={{ border: "1.5px solid #e2e8f0", borderRadius: 10, padding: "9px 12px", fontSize: 13, color: "#475569", background: "#fff" }} />
              </>
            )}
            {expHasFilters && (
              <button onClick={clearExpFilters} style={{ background: "none", border: "none", color: "#94a3b8", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                <X size={12} /> Clear filters
              </button>
            )}
            <button onClick={() => { setExpEditId(null); setExpForm(emptyExp); setShowExpModal(true); }} style={{ marginLeft: "auto", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 10, padding: "9px 16px", fontWeight: 600, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {expHasFilters && (
            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 14 }}>
              {filteredExp.length} of {propExpenses.length} expenses
              <span style={{ color: "#b91c1c", fontWeight: 600, marginLeft: 8 }}>Total: {fmt(expTotal)}</span>
            </p>
          )}
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9" }}>
            {filteredExp.length === 0 ? (
              <div style={{ textAlign: "center", padding: 40, color: "#94a3b8" }}>
                <DollarSign size={32} style={{ margin: "0 auto 12px", display: "block" }} />
                {expHasFilters ? (
                  <>
                    <p style={{ fontWeight: 600, marginBottom: 4 }}>No expenses match your filters</p>
                    <button onClick={clearExpFilters} style={{ background: "none", border: "none", color: "#3b82f6", fontSize: 13, cursor: "pointer", textDecoration: "underline" }}>Clear filters</button>
                  </>
                ) : (
                  <>
                    <p style={{ fontWeight: 600, marginBottom: 4 }}>No expenses logged yet</p>
                    <p style={{ fontSize: 13 }}>Click "Add Expense" to start tracking costs for this property.</p>
                  </>
                )}
              </div>
            ) : (
                ...
                <thead>
                  <tr>
                    {["Date", "Category", "Description", "Paid To", "Amount", ""].map(h => (
                      <th key={h} style={{ padding: "10px 14px", textAlign: "left", color: "#94a3b8", fontSize: 12, fontWeight: 600, textTransform: "uppercase", borderBottom: "1px solid #f1f5f9" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredExp.map(t => (
                    <tr key={t.id} style={{ borderBottom: "1px solid #f8fafc", transition: "background 0.15s" }}
                      onMouseEnter={e => e.currentTarget.style.background = "#f8fafc"}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <td style={{ padding: "12px 14px", fontSize: 13, color: "#64748b" }}>{t.date}</td>
                      <td style={{ padding: "12px 14px" }}>
                        <span style={{ background: "#f1f5f9", borderRadius: 6, padding: "3px 8px", fontSize: 12, fontWeight: 600, color: "#475569" }}>{t.category}</span>
                      </td>
                      <td style={{ padding: "12px 14px", fontSize: 13, color: "#0f172a" }}>{t.description}</td>
                      <td style={{ padding: "12px 14px", fontSize: 13, color: "#64748b" }}>{t.payee || "—"}</td>
                      <td style={{ padding: "12px 14px", fontSize: 14, fontWeight: 700, color: "#b91c1c" }}>{fmt(Math.abs(t.amount))}</td>
                      <td style={{ padding: "12px 14px", textAlign: "right" }}>
                        <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                          <button onClick={() => openExpEdit(t)} style={{ background: "#f1f5f9", border: "none", borderRadius: 7, padding: "5px 8px", cursor: "pointer", color: "#475569", display: "flex", alignItems: "center" }} title="Edit"><Pencil size={12} /></button>
                          <button onClick={() => setExpDeleteConfirm(t)} style={{ background: "#fee2e2", border: "none", borderRadius: 7, padding: "5px 8px", cursor: "pointer", color: "#ef4444", display: "flex", alignItems: "center" }} title="Delete"><Trash2 size={12} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
              <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 500, boxShadow: "0 20px 60px rgba(0,0,0,0.18)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
                  <h2 style={{ color: "#0f172a", fontSize: 19, fontWeight: 700 }}>{expEditId ? "Edit Expense" : "Add Expense"}</h2>
                  <button onClick={() => { setShowExpModal(false); setExpEditId(null); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}><X size={20} /></button>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 5 }}>Date *</p>
                      <input type="date" style={iS} value={expForm.date} onChange={sef("date")} />
                    </div>
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 5 }}>Amount *</p>
                      <input type="number" style={iS} placeholder="0.00" value={expForm.amount} onChange={sef("amount")} />
                    </div>
                  </div>
                  <div>
                    <p style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 5 }}>Category *</p>
                    <select style={iS} value={expForm.category} onChange={sef("category")}>
                      {Object.entries(PROP_EXPENSE_GROUPS).map(([group, subs]) => (
                  ...
                  <div>
                    ...
                    <input style={iS} placeholder="What was this expense for?" value={expForm.description} onChange={sef("description")} />
                  </div>
                  <div style={{ position: "relative" }}>
                    <p style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 5 }}>Paid To</p>
                    <input style={iS} placeholder="Start typing to search or add new..." value={expForm.payee}
                      onChange={e => { setExpForm(f => ({ ...f, payee: e.target.value })); setPayeeFocus(true); }}
                      onFocus={() => setPayeeFocus(true)} onBlur={() => setTimeout(() => setPayeeFocus(false), 150)} />
                    {!payeeFocus && !expForm.payee && <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 4, fontStyle: "italic" }}>Type to search previous payees or add new</p>}
                    {payeeFocus && (() => {
                      const q = expForm.payee.toLowerCase();
                      const matches = q ? allPayees.filter(p => p.toLowerCase().includes(q) && p.toLowerCase() !== q) : allPayees.slice(0, 6);
                      ...
                      return (
                        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.10)", zIndex: 200, overflow: "hidden", maxHeight: 200, overflowY: "auto" }}>
                          {matches.slice(0, 6).map(p => (
                            <button key={p} onMouseDown={() => { setExpForm(f => ({ ...f, payee: p })); setPayeeFocus(false); }}
                              ...
                              <Users size={13} style={{ color: "#94a3b8", flexShrink: 0 }} /><span>{p}</span>
                            </button>
                          ))}
                          {showNew && (
                            <button onMouseDown={() => { setExpForm(f => ({ ...f, payee: f.payee })); setPayeeFocus(false); }}
                              ...
                              <span style={{ fontSize: 13, color: "#f59e0b", fontWeight: 600 }}>Add &ldquo;{expForm.payee}&rdquo; as new</span>
                            </button>
                          )}
                        </div>
                      );
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
                  <button onClick={handleExpSave} disabled={!expForm.description.trim() || !expForm.amount || !expForm.date} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: !expForm.description.trim() || !expForm.amount || !expForm.date ? "#e2e8f0" : "#3b82f6", color: "#fff", fontWeight: 700, fontSize: 14, cursor: !expForm.description.trim() || !expForm.amount || !expForm.date ? "not-allowed" : "pointer" }}>{expEditId ? "Save Changes" : "Add Expense"}</button>
                  <button onClick={() => { setShowExpModal(false); setExpEditId(null); }} style={{ padding: "11px 18px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer", color: "#64748b" }}>Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          {expDeleteConfirm && (
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
              <div style={{ background: "#fff", borderRadius: 20, width: 440, padding: 28 }}>
                <h2 style={{ color: "#0f172a", fontSize: 18, fontWeight: 700, marginBottom: 14 }}>Delete Expense</h2>
                <p style={{ color: "#475569", fontSize: 14, marginBottom: 8 }}>Are you sure you want to remove this expense?</p>
                <div style={{ background: "#f8fafc", borderRadius: 10, padding: 14, marginBottom: 18 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#0f172a" }}>{expDeleteConfirm.description}</p>
                  <p style={{ fontSize: 12, color: "#64748b" }}>{expDeleteConfirm.date} · {fmt(Math.abs(expDeleteConfirm.amount))}</p>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={handleExpDelete} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: "#ef4444", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Delete</button>
                  <button onClick={() => setExpDeleteConfirm(null)} style={{ padding: "11px 18px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", fontWeight: 600, fontSize: 14, cursor: "pointer", color: "#64748b" }}>Cancel</button>
                </div>
              </div>
            </div>
          )}
